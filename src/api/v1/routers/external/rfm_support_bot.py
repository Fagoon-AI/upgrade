...
from src.prompts import RFM_SUPPORT_BOT_FAILED_RESPONSE, RFM_SUPPORT_BOT_SYSTEM_PROMPT
from src.constants import RFM_SUPPORT_BOT_COLLECTION_NAME

router = APIRouter(prefix="")


@router.post(path="/bot")
async def support_bot(input_request: InputRequestBody):
    ...
    try:
        bot_service = SupportBotService(
            collection_name=RFM_SUPPORT_BOT_COLLECTION_NAME,
        )

        response = await bot_service.get_answer_from_support_bot(
            user_query=user_query,
            system_prompt=RFM_SUPPORT_BOT_SYSTEM_PROMPT,
            fallback_handle_response=RFM_SUPPORT_BOT_FAILED_RESPONSE,
        )
