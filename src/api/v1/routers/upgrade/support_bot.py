...
from src.prompts import SUPPORT_BOT_SYSTEM_PROMPT, SUPPORT_BOT_FAILED_RESPONSE
from src.constants import UPGRADE_SUPPORT_BOT_COLLECTION_NAME

router = APIRouter(prefix="")


@router.post(path="/agent")
async def support_bot(input_request: InputRequestBody):
    ...
    try:
        bot_service = SupportBotService(
            collection_name=UPGRADE_SUPPORT_BOT_COLLECTION_NAME
        )

        response = await bot_service.get_answer_from_support_bot(
            user_query=user_query,
            system_prompt=SUPPORT_BOT_SYSTEM_PROMPT,
            fallback_handle_response=SUPPORT_BOT_FAILED_RESPONSE,
        )
