...
class AgentManager:
    ...
    async def get_agent(self, agent_id: str) -> Optional[Dict[str, Any]]:
        async with self.postgres_manager.get_session() as session:
            pg_services = PostgresServices(session)
            sql_agent = await pg_services.get_agent_by_id(uuid.UUID(agent_id))
    ...
    async def list_agents(self, user_id: str) -> List[Dict[str, Any]]:
        async with self.postgres_manager.get_session() as session:
            pg_services = PostgresServices(session)
            sql_agents = await pg_services.get_all_agents_by_user_id(uuid.UUID(user_id))
            return [
                {
                    "id": str(agent.id),
                    "name": agent.name,
    ...
    async def update_agent(self, agent_id: str, update_data: AgentUpdateModel) -> bool:
        async with self.postgres_manager.get_session() as session:
            ...
            return await pg_services.update_agent(uuid.UUID(agent_id), update_dict)

    async def delete_agent(self, agent_id: str) -> bool:
        async with self.postgres_manager.get_session() as session:
            pg_services = PostgresServices(session)
            return await pg_services.delete_agent(uuid.UUID(agent_id))
