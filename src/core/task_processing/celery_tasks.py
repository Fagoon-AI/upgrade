...
async def _generate_video_job_async(
    ...
    try:
        ...
        logger.info(f"Job {job_id} successfully updated to PROCESSING.")

        logger.info(f"Job {job_id}: Initializing video generator...")
        veo_config = VeoVideoGeneratorConfig(
            model="veo-2.0-generate-001",
        )
        video_generator = VeoVideoGenerator(config=veo_config)
        logger.info(f"Job {job_id}: Calling video generator for video generation...")

        video_output_path_or_url = video_generator.generate_video(video_path, job_id, final_prompt_for_video)

        logger.info(
            f"Job {job_id}: Video generation complete. Output at: {video_output_path_or_url}"
        )
        ...
        logger.info(f"Job {job_id} successfully updated to COMPLETED with output path/URL.")

    except VideoGenerationError as e:
        logger.error(f"Video generation specific error for job_id {job_id}: {e}", exc_info=True)
        error_detail = str(e)
        ...
        logger.error(f"Job {job_id} updated to FAILED with error: {error_detail}")

    except Exception as e:
        logger.error(f"Unexpected error in Celery task for job_id {job_id}: {e}", exc_info=True)
        fail_data = {"status": "FAILED", "error_message": f"Unexpected error: {str(e)}"}
        await db_services.update_video_job(uuid.UUID(job_id) if len(job_id) == 36 else None, fail_data)
        logger.error(f"Job {job_id} updated to FAILED with unexpected error: {str(e)}")
...
def generate_video_task(
    ...
    logger.info(f"Celery task {self.request.id} started for job_id: {job_id}")
    ...
    try:
        asyncio.run(main())
        logger.info(f"Celery task {self.request.id} finished for job_id: {job_id}")
    except Exception as e:
        logger.error(f"Error running async logic in synchronous Celery task {job_id}: {e}", exc_info=True)
        ...
        try:
            ...
            logger.critical(f"FAILED to update job {job_id} status to FAILED: {db_err}", exc_info=True)
    finally:
