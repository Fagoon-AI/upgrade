...
async def send_mail(to_email: str, url: str, email_type: str):
    ...
    if email_type == "signup":
        ...
        logger.error("Invalid email type: {}", email_type)
    ...
    try:
        logger.info("Attempting to connect to {}:{} with user {}", system_setting.EMAIL_HOST, system_setting.EMAIL_PORT, system_setting.EMAIL_USERNAME)
        ...
        logger.error("SMTP Authentication Error for {}: {}", to_email, e)
        raise AppError(f"Authentication failed for email server: {e}", 500)
    except aiosmtplib.SMTPConnectError as e:
        logger.error("SMTP Connection Error for {}: {}", to_email, e)
